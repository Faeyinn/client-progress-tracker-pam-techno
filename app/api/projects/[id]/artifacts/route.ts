...
export async function GET(
  ...
  try {
    ...
    const payload = artifacts.map((a) => ({
      ...
      mimeType: a.mimeType,
      fileSize: a.fileSize,
      fileUrl: a.fileData ? `/api/projects/${projectId}/artifacts/${a.id}/file` : null,
    }));
    ...
    return NextResponse.json({ message: "Failed to fetch artifacts" }, { status: 500 });
  }
}
...
export async function POST(
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      phase = typeof form.get("phase") === "string" ? String(form.get("phase")) : null;
      type = typeof form.get("type") === "string" ? String(form.get("type")) : null;
      sourceLinkUrl = typeof form.get("sourceLinkUrl") === "string" ? String(form.get("sourceLinkUrl")) : null;
      ...
      sourceLinkUrl = typeof body?.sourceLinkUrl === "string" ? body.sourceLinkUrl : null;
    }
    ...
    if (!phase || !isProjectPhase(phase)) {
      return NextResponse.json({ message: "Valid project phase is required" }, { status: 400 });
    }

    if (type && !isArtifactType(type)) {
      return NextResponse.json({ message: "Invalid artifact type" }, { status: 400 });
    }

    const phaseValue = phase as PrismaProjectPhase;
    const typeValue = type && isArtifactType(type) ? (type as PrismaArtifactType) : null;
    ...
    return NextResponse.json(
      {
        ...
        sourceLinkUrl: created.sourceLinkUrl,
        fileName: created.fileName,
        mimeType: created.mimeType,
        fileSize: created.fileSize,
        fileUrl: created.fileData ? `/api/projects/${projectId}/artifacts/${created.id}/file` : null,
      },
      { status: 201 },
    );
  } catch (error) {
    ...
    return NextResponse.json({ message: "Failed to create artifact" }, { status: 500 });
  }
}
